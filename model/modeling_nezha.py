...
class NeZhaForMultipleChoiceWithMatch(NeZhaPreTrainedModel):

    def __init__(self, config, num_choices=2):
        super(NeZhaForMultipleChoiceWithMatch, self).__init__(config)
...
# DUMA zuijia
class DUMAZuiJia(nn.Module):
    def __init__(self, config):
        super(DUMAZuiJia, self).__init__()
        self.attention = NeZhaSelfAttention(config)
        self.pooler = MeanPooler(config)
        self.outputlayer = BertSelfOutput(config)
...
class NeZhaForMultipleChoiceWithDUMAZuiJia(NeZhaPreTrainedModel):
    def __init__(self, config, num_choices=2):
        super(NeZhaForMultipleChoiceWithDUMAZuiJia, self).__init__(config)
        self.num_choices = num_choices
        self.bert = NeZhaModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        # self.duma = DUMA(config)
        duma = DUMAZuiJia(config)
...
# DUMA
class DUMA256(nn.Module):
    def __init__(self, config):
        super(DUMA256, self).__init__()
        self.attention = NeZhaSelfAttention(config)
        self.pooler = MeanPooler(config)
        self.outputlayer = BertSelfOutput(config)
...
# 2层 256
class NeZhaForMultipleChoiceWithDUMA256(NeZhaPreTrainedModel):
    def __init__(self, config, num_choices=2):
        super(NeZhaForMultipleChoiceWithDUMA256, self).__init__(config)
        self.num_choices = num_choices
        self.bert = NeZhaModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.dumas = nn.ModuleList([DUMA256(config) for _ in range(2)])
        self.pooler = self.bert.pooler
        self.classifier = nn.Linear(config.hidden_size, 1)
        self.init_weights()
