...
#         self.attention = NeZhaSelfAttention(config)
#         # self.pooler = MeanPooler(config)
#         self.pooler = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
#         self.outputlayer = BertSelfOutput(config)
#
...
class DUMAOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        ...
        hidden_states = torch.cat([torch.unsqueeze(hidden_states, -1), torch.unsqueeze(input_tensor, -1)], axis=2)
